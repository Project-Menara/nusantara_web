<template>
  <Menu as="div" class="relative inline-block text-left">
    <div>
      <MenuButton :class="buttonClass" class="inline-flex justify-center w-28 rounded-md border px-4 py-2 text-sm font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-violet-500">
      ...
      <MenuItems class="absolute right-0 z-10 mt-2 w-28 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
        <div class="py-1">
          <MenuItem v-if="!props.item.isActive" v-slot="{ active }">
            <a href="#" @click.prevent="emit('toggle')" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-sm']">
              Aktif
            </a>
          </MenuItem>
          <MenuItem v-if="props.item.isActive" v-slot="{ active }">
...
const buttonText = computed(() => props.item.isActive ? 'Aktif' : 'Tidak Aktif');
