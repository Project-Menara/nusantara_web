        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center">
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
            ...
            <DialogPanel tabindex="-1" :class="[
              ...
              maxWidthClass // <-- GANTI `max-w-md` DENGAN INI
            ]">
              <slot name="header">
                <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900 dark:text-gray-500">
                  Judul Modal Default
                </DialogTitle>
              </slot>
              ...
              <div v-if="!hideFooter" class="mt-6 flex justify-end space-x-3">
                <slot name="footer">
                  <div v-if="loading" class="flex items-center justify-center w-full">
                    <svg class="animate-spin h-5 w-5 text-[var(--color-violet-600)]" xmlns="http://www.w3.org/2000/svg"
...
const props = defineProps({ // Ubah defineProps menjadi const props
...
// TAMBAHKAN COMPUTED PROPERTY INI
const maxWidthClass = computed(() => {
  switch (props.size) {
    ...
    default: return 'max-w-md'; // Fallback default
  }
});
</script>
