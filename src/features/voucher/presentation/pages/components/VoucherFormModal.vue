...
<template>
  <BaseModal :isOpen="voucherStore.isFormModalOpen" @close="closeFormModal">
    ...
    <template #body>
      <div class="relative">
        <div
          v-if="isEditMode && voucherStore.isFormLoading"
        ...
        <form @submit.prevent="handleSubmit" class="mt-6 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label for="discountType" class="block text-sm font-medium mb-1"
                >Tipe Diskon</label
              >
              <select
          ...
          <div v-if="formData.discountType === 'percent'">
            <label for="discountPercent" class="block text-sm font-medium mb-1"
          ...
          <div v-if="formData.discountType === 'amount'">
            <label for="discountAmount" class="block text-sm font-medium mb-1"
              >Jumlah Diskon (Rp)</label
            >
            <input
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label for="minimumSpend" class="block text-sm font-medium mb-1"
                >Minimal Belanja (Rp)</label
              >
              <input
                v-model.number="formData.minimumSpend"
                type="number"
                id="minimumSpend"
                class="form-input w-full"
              />
            ...
            <div>
              <label for="pointCost" class="block text-sm font-medium mb-1"
                >Biaya Poin</label
              >
              <input
            ...
            <div>
              <label for="quota" class="block text-sm font-medium mb-1"
                >Kuota</label
              >
              <input
    ...
    <template #footer>
      ...
      <div class="relative inline-block group">
        <button
          @click="handleSubmit"
          :disabled="voucherStore.isFormLoading"
          class="btn bg-violet-600 ..."
        >
...
const handleSubmit = () => {
  ...
  const payload = {
    ...
    // ✅ Mapping dari camelCase ke snake_case
    minimum_spend: formData.value.minimumSpend,
    point_cost: formData.value.pointCost,

    // Format tanggal ke ISO String
    start_date: new Date(formData.value.startDate).toISOString(),
    end_date: new Date(formData.value.endDate).toISOString(),

    // Status hanya dikirim saat mode Tambah
    ...(!isEditMode.value && { status: 1 }),
  };

  // Hapus properti yang tidak relevan
  // ✅ Logika diskon yang disempurnakan
