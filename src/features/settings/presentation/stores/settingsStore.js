...
import { IncorrectPasswordFailure } from "@/core/error/failures";

export const useSettingsStore = defineStore("settings", () => {
  const isLoading = ref(false);
  const modalStore = useModalStore();
  const authStore = useAuthStore(); // Untuk mengambil data user
  ...
  async function updatePassword(passwords) {
    ...
    if (result.left) {
      ...
      let message = (failure instanceof IncorrectPasswordFailure) 
