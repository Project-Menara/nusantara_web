...
const originalData = ref(null); // Untuk perbandingan di mode edit

// --- State Gambar (disesuaikan seperti ProductForm) ---
const coverFile = ref(null);
const coverPreview = ref('');
const galleryItems = ref([]); // { file: File|null, url: string, isExisting: boolean }
const deletedGalleryUrls = ref(new Set()); // Untuk melacak gambar lama yang dihapus
...
const isStepValid = computed(() => {
    ...
    if (currentStep.value === 3 && !isEditMode.value) {
        return !!coverFile.value; // Wajib ada cover saat create
    }
    return true;
});
...
function populateFormForEdit(shop) {
    ...
    originalData.value = JSON.parse(JSON.stringify({ formData: formData.value, selectedCashierIds: selectedCashierIds.value, selectedProducts: selectedProducts.value }));
}
...
function removeGalleryImage(index) {
    ...
    // Jika yang dihapus adalah gambar lama dari server, catat URL-nya
...
async function handleSubmit() {
    ...
    // Append data teks dan data JSON
    ...
    // --- ✅ LOGIKA COVER & GALERI DISESUAIKAN SEPERTI PRODUCTFORM ---
    if (!isEditMode.value) { // Mode Create
        data.append('status', 1);
        if (coverFile.value) data.append('cover', coverFile.value);
        galleryItems.value.forEach(item => {
            if (item.file) data.append('galerry', item.file); // Typo 'galerry' disesuaikan dengan API spec Anda
        });
    } else { // Mode Edit
        ...
        if (hasNewGalleryFiles || hasDeletedGalleryImages) {
            ...
            for (const item of galleryItems.value) {
                if (item.file) { // Ini file baru dari komputer
                    data.append('new_gallery', item.file);
                } else { // Ini file lama yang dipertahankan
