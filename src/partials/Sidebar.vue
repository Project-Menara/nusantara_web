      ...
      <div v-else>
        ...
        <div class="pt-3 hidden lg:flex 2xl:hidden justify-end mt-auto">
          <button @click.prevent="sidebarExpanded = !sidebarExpanded"
            ...
            <ChevronRightIcon class="w-5 h-5 transition-transform sidebar-expanded:rotate-180" />
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Ambil role pengguna dari store secara dinamis
    const userRole = computed(() => authStore.user?.role || "guest");

    // Konfigurasi semua kemungkinan menu
    const menuConfig = ref([
      ...
      },
      {
        ...
        items: [
          ...
          {
            name: "Daftar-Produk-Toko",
            path: ROUTE_PATHS.SHOP_PRODUCTS,
            iconComponent: ArchiveBoxIcon,
            roles: ["admin"],
          },
          {
    ...
    // Filter menu berdasarkan role pengguna dari store
    const filteredMenu = computed(() => {
      const processedGroups = menuConfig.value.map((group) => {
        if (group.type !== "group") {
          return group.roles.includes(userRole.value) ? group : null;
        }
